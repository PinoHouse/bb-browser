      ...
      capture("network", async () => {
        const response = await command(
          ...
          },
          {
            timeoutMs: 60_000,
            idempotency:
              input.command === "requests" ? "read" : "safe_write",
          },
        );
        return textResult(
...
export function registerBrowserTools(
  ...
  server.tool("browser_click", "Click an element by ref", {
    ref: z.string().describe("Element ref from snapshot"),
    tab: z.number().optional().describe("Tab ID to target"),
  }, handlers.browser_click);
  server.tool("browser_fill", "Fill text into an input", {
    ref: z.string().describe("Element ref from snapshot"),
    text: z.string().describe("Text to fill"),
    tab: z.number().optional().describe("Tab ID to target"),
  }, handlers.browser_fill);
  server.tool("browser_type", "Type text into an input without clearing", {
    ref: z.string().describe("Element ref from snapshot"),
    text: z.string().describe("Text to type"),
    tab: z.number().optional().describe("Tab ID to target"),
  }, handlers.browser_type);
  server.tool("browser_open", "Navigate to a URL", {
    url: z.string().describe("URL to open"),
    tab: z.number().optional().describe("Tab ID to target"),
  }, handlers.browser_open);
  server.tool("browser_tab_list", "List all tabs", {}, handlers.browser_tab_list);
  server.tool("browser_tab_new", "Open a new tab", {
  ...
  server.tool("browser_eval", "Execute JavaScript in page context", {
    script: z.string().describe("JavaScript source to execute"),
    tab: z.number().optional().describe("Tab ID to target"),
  }, handlers.browser_eval);
  server.tool("browser_network", "Inspect or clear network activity", {
  ...
  server.tool("browser_get", "Get element text or attribute", {
    attribute: z.enum(["text", "url", "title", "value", "html"]),
    ref: z.string().optional(),
    tab: z.number().optional(),
  }, handlers.browser_get);
  server.tool("browser_close", "Close the current or specified tab", {
    tab: z.number().optional(),
  ...
  server.tool("browser_wait", "Wait for a number of milliseconds", {
    time: z.number(),
    tab: z.number().optional(),
